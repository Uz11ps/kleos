package com.kleos.education.ui.auth
...
import com.kleos.education.data.auth.AuthRepository
import com.kleos.education.databinding.ActivityAuthBinding
import com.kleos.education.databinding.BottomSheetLoginBinding
import com.kleos.education.databinding.BottomSheetRegisterBinding
import com.kleos.education.databinding.BottomSheetForgotPasswordBinding
import android.content.Intent
import com.kleos.education.MainActivity
import com.kleos.education.databinding.DialogInviteBinding
...
class AuthActivity : AppCompatActivity() {
    ...
    private fun setupWelcomeScreen() {
        ...
        com.kleos.education.ui.utils.AnimationUtils.slideUpFade(binding.welcomeTitle, 600, 0)
        com.kleos.education.ui.utils.AnimationUtils.slideUpFade(binding.welcomeSubtitle, 500, 100)
        com.kleos.education.ui.utils.AnimationUtils.slideUpFade(binding.bottomBar, 500, 200)
        com.kleos.education.ui.utils.AnimationUtils.slideUpFade(binding.menuButton, 400, 0)
        
        // Обработчик клика на бургер-меню
        binding.menuButton.setOnClickListener {
            val intent = Intent(this, com.kleos.education.ui.language.LanguageActivity::class.java)
        ...
        binding.submitButton.text = getString(com.kleos.education.R.string.action_login)
        binding.toggleModeButton.text = getString(com.kleos.education.R.string.action_register)
        ...
        binding.guestLoginText.setOnClickListener {
            ...
            session.saveUser(fullName = getString(com.kleos.education.R.string.guest), email = "guest@local")
    ...
    private fun showFormScreen() {
        ...
        binding.root.setBackgroundColor(getColor(com.kleos.education.R.color.white))
        
        // Настраиваем обработчики кнопок для формы
        setupFormButtons()
        
        // Анимации появления элементов формы
        com.kleos.education.ui.utils.AnimationUtils.slideUpFade(binding.titleText, 600, 0)
        com.kleos.education.ui.utils.AnimationUtils.slideUpFade(binding.emailInputLayout, 500, 100)
        com.kleos.education.ui.utils.AnimationUtils.slideUpFade(binding.passwordInputLayout, 500, 200)
        com.kleos.education.ui.utils.AnimationUtils.slideUpFade(binding.forgotPasswordText, 400, 300)
        com.kleos.education.ui.utils.AnimationUtils.slideUpFade(binding.bottomBar, 500, 400)
    ...
    private fun proceedToMain(showInviteFallback: Boolean = false) {
        ...
        startActivity(intent)
        overridePendingTransition(com.kleos.education.R.anim.slide_in_from_right, com.kleos.education.R.anim.fade_out)
    }
    ...
    private fun renderMode() {
        ...
        if (showFullName && binding.fullNameInputLayout.visibility != View.VISIBLE) {
            ...
            com.kleos.education.ui.utils.AnimationUtils.slideUpFade(binding.fullNameInputLayout, 400)
        } else if (!showFullName && binding.fullNameInputLayout.visibility == View.VISIBLE) {
            com.kleos.education.ui.utils.AnimationUtils.fadeOutScale(binding.fullNameInputLayout, 300) {
                binding.fullNameInputLayout.visibility = View.GONE
            }
        }
        
        val underline = binding.root.findViewById<View>(com.kleos.education.R.id.fullNameUnderline)
        underline?.visibility = if (showFullName) View.VISIBLE else View.GONE
        
        // Анимация изменения заголовка
        com.kleos.education.ui.utils.AnimationUtils.rotateIn(binding.titleText, 400)
        binding.submitButton.text = if (isRegisterMode) {
            getString(com.kleos.education.R.string.action_register)
        } else {
            getString(com.kleos.education.R.string.action_login)
        }
        // Кнопка переключения режима показывает короткий текст целевого режима
        binding.toggleModeButton.text = if (isRegisterMode) {
            getString(com.kleos.education.R.string.action_login) // "Войти"
        } else {
            getString(com.kleos.education.R.string.action_register) // "Зарегистрироваться"
        }
        binding.titleText.text = if (isRegisterMode) this@AuthActivity.t(com.kleos.education.R.string.title_sign_up) else this@AuthActivity.t(com.kleos.education.R.string.title_sign_in)
        ...
        binding.toggleModeButton.setStrokeColorResource(com.kleos.education.R.color.kleos_blue)
        binding.toggleModeButton.setTextColor(getColor(com.kleos.education.R.color.kleos_blue))
        binding.submitButton.setBackgroundTintList(android.content.res.ColorStateList.valueOf(getColor(com.kleos.education.R.color.kleos_blue)))
        ...
        lifecycleScope.launch {
            ...
            if (result.isSuccess) {
                ...
                if (serverError == "email_not_verified") {
                    val intent = Intent(this@AuthActivity, com.kleos.education.ui.verify.VerifyEmailActivity::class.java)
    ...
    private fun applyBottomSheetStyle(bottomSheetDialog: BottomSheetDialog) {
        ...
        bottomSheetDialog.setOnShowListener {
            ...
            bottomSheet?.let { view ->
                val cornerRadius = resources.getDimension(com.kleos.education.R.dimen.bottom_sheet_corner_radius)
    ...
    private fun showLoginBottomSheet() {
        val bottomSheetDialog = BottomSheetDialog(this, com.kleos.education.R.style.Theme_Kleos_BottomSheetDialog)
        ...
        sheetBinding.registerLink.text = getString(com.kleos.education.R.string.no_account_register)
    ...
    private fun showRegisterBottomSheet() {
        ...
        sheetBinding.loginLink.text = getString(com.kleos.education.R.string.have_account_login)
    ...
    private fun showForgotPasswordBottomSheet() {
        ...
        sheetBinding.resetPasswordButton.setOnClickListener {
            ...
            Toast.makeText(this, getString(com.kleos.education.R.string.forgot_password_not_impl), Toast.LENGTH_SHORT).show()
    ...
    private fun showInviteDialog(onClose: () -> Unit) {
        ...
        dialogBinding.inviteButton.setOnClickListener {
            val shareIntent = Intent(Intent.ACTION_SEND).apply {
                type = "text/plain"
                putExtra(Intent.EXTRA_TEXT, getString(com.kleos.education.R.string.invite_share_text))
            }
            startActivity(Intent.createChooser(shareIntent, getString(com.kleos.education.R.string.invite_share_title)))
            dialog.dismiss()
            onClose()
        }
        dialog.show()
