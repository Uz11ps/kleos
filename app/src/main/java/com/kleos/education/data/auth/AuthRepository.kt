package com.kleos.education.data.auth

import android.content.Context
import com.kleos.education.data.model.User
...
interface AuthRepository {
    ...
    class Http(private val context: Context) : AuthRepository {
        private val sessionManager = SessionManager(context)
        private val api = com.kleos.education.data.network.ApiClient.retrofit
            .create(com.kleos.education.data.network.AuthApi::class.java)
        ...
        override suspend fun login(email: String, password: String): Result<User> = runCatching {
            val resp = api.login(com.kleos.education.data.network.LoginRequest(email, password))
            ...
            com.kleos.education.utils.FcmTokenManager.registerToken(context)
            sessionManager.getCurrentUser()!!
        }

        override suspend fun register(fullName: String, email: String, password: String): Result<User> = runCatching {
            val resp = api.register(com.kleos.education.data.network.RegisterRequest(fullName, email, password))
