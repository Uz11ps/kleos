...
// API endpoint for getting countries list (public) - поддерживаем оба варианта
...
// API endpoint for getting consent text (public) - поддерживаем оба варианта
...
router.get('/settings/consent/:lang', async (req, res) => {
  const { Settings } = await import('../models/Settings.js');
  const lang = req.params.lang === 'ru' ? 'ru' : 'en';
  const key = `consent_text_${lang}`;
  const consentSetting = await Settings.findOne({ key }).lean();
